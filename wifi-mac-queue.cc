...
{
	//std::cout<<"WQ Constructor"<<std::endl;
	//SetMaxPackets(200);
}
...
WifiMacQueue::Enqueue (Ptr<WifiMacQueueItem> item)
{
 ...
 // std::cout<<"packet size in queue"<<GetNPackets ()<<" Max "<<GetMaxPackets ()<<std::endl;//palash
   
  
  if(GetNPackets () > GetMaxPackets ()*0.85)
   {    //std::cout<<"Mark ECN size "<<GetNPackets()<<" Max "<<GetMaxPackets ()<<std::endl;
        Ptr<Packet> mepacket = new Packet (*item->GetPacket());  //palash
        //std::cout<<"ECN marked now.... in the queue"<<std::endl;
        Ipv4Header ipHeader;
        LlcSnapHeader llc1;
        mepacket->RemoveHeader(llc1); 
        if (mepacket->PeekHeader (ipHeader) != 0 ) 
        {
         mepacket->RemoveHeader(ipHeader); 
         ipHeader.EnableChecksum();
         ipHeader.SetEcn(Ipv4Header::ECN_CE);
         mepacket->AddHeader(ipHeader);
         mepacket->AddHeader(llc1);

         Ptr<WifiMacQueueItem> meitem =new WifiMacQueueItem(mepacket, item->GetHeader());
         return DoEnqueue (Tail (), meitem);
         }
         else
            return DoEnqueue (Tail (), item);
  }
